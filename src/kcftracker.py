...
def rearrange(img):
	...
	if img.shape[1] % 2 != 0:
		...
		shape_1 = img.shape[1]
	xh, yh = shape_1//2, shape_0//2
	img_[0:yh,0:xh], img_[yh:shape_0,xh:shape_1] = img[yh:shape_0,xh:shape_1], img[0:yh,0:xh]
	img_[0:yh,xh:shape_1], img_[yh:shape_0,0:xh] = img[yh:shape_0,0:xh], img[0:yh,xh:shape_1]
	return img_
...
class KCFTracker:
	...
	def getFeatures(self, image, inithann, scale_adjust=1.0):
		...
		if(inithann):
			...
			if(self._hogfeatures):
				self._tmpl_sz[0] = int(int(self._tmpl_sz[0]) / (2*self.cell_size) * 2*self.cell_size + 2*self.cell_size) #change point
				self._tmpl_sz[1] = int(int(self._tmpl_sz[1]) / (2*self.cell_size) * 2*self.cell_size + 2*self.cell_size) #change point
			else:
				self._tmpl_sz[0] = int(int(self._tmpl_sz[0]) / 2 * 2) #change point
				self._tmpl_sz[1] = int(int(self._tmpl_sz[1]) / 2 * 2) #change point
	...
	def init(self, roi, image):
		...
		self._roi = list(self._roi) #change point
